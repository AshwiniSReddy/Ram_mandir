...
export const QuizScreen: React.FC<QuizScreenProps> = ({
  ...
  const handleAnswer = async (index: number) => {
    ...
    if (isCorrect) {
      ...
      // Set waiting state to display the waiting message
      setWaitingForBackend(true);
      // Wait for 5 seconds then call onAnswer as final
      setTimeout(() => {
        setWaitingForBackend(false);
        setIsAnswerProcessing(true);
        onAnswer(true, timeRemaining, true);
  ...
  // Cast the translated options to a string array to enable .map
